...
public class DataView extends View {
    ...
    protected void onDraw(Canvas canvas) {
        ...
        for (int i=0; i<dataList.size(); i++) {
            if (i > 0) {
                canvas.drawLine((i-1)*10, dataList.get(i-1)*1000, i*10, dataList.get(i)*1000, paint);
            }
        }
//        float timeline = 0;
...
//        }
    ...
    private float normalize(float num, float max) {
        return height / 2 - num / max * height / 2;
    }
}
